...
class AppsFlyerEvent extends Model
{
    ...
    public function getStats(array $filters = []): array
    {
        $sql = 'SELECT 
                COUNT(*) as total,
                SUM(CASE WHEN af_status = :pending THEN 1 ELSE 0 END) as pending,
                SUM(CASE WHEN af_status = :received THEN 1 ELSE 0 END) as received,
                SUM(CASE WHEN af_status = :validated THEN 1 ELSE 0 END) as validated,
                SUM(CASE WHEN af_status = :invalid THEN 1 ELSE 0 END) as invalid
            FROM appsflyer_events
            WHERE 1=1';
        $params = [
            'pending' => AppsFlyerStatus::PENDING->value,
            'received' => AppsFlyerStatus::RECEIVED->value,
            'validated' => AppsFlyerStatus::VALIDATED->value,
            'invalid' => AppsFlyerStatus::INVALID->value,
        ];
